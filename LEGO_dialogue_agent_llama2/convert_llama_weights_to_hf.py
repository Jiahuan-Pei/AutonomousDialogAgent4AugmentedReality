...
"""
This script is to convert raw llama2 to huggingface model, small changes from the original script: https://github.com/huggingface/transformers/blob/main/src/transformers/models/llama/convert_llama_weights_to_hf.py
"""
import argparse
import gc
import json
...
def main(mode='-chat'):
    ...
    parser.add_argument(
        ...
        default="-chat",
        choices=["", "-chat", "-chathf"],
        help="to construct the real model forder name.",
    )
    args = parser.parse_args()
    if args.model_size != "tokenizer_only":
        write_model(
            model_path=args.output_dir,
            input_base_path=os.path.join(args.input_dir, f"llama-2-{args.model_size.lower()}{args.mode}"),
